...
export class LayaDCCTools {
    ...
    static async genZipByComparePath(dccold, dccnew, outPath, outFile) {
        ...
        let zipPath = path.join(outPath, outFile !== null && outFile !== void 0 ? outFile : "dcc.zip");
        ...
        for (let objid of changed) {
            try {
                ...
                let file = path.join(dccnew, await dccclientNew.getObjectUrl(objid));
                console.log(file);
                let buf = await dccclientNew.fileIO.read(await dccclientNew.getObjectUrl(objid), 'buffer', false);
