import { AppResReader_Native } from '../assets/LayaDCC/common/AppResReader_Native.js';
import { DCCUpdate, UniDCCClient } from './DCCUpdate.js';
import { Zip_Native } from '../assets/LayaDCC/common/Zip_Native.js';
import { Env } from '../assets/LayaDCC/common/Env.js';
import { LayaDCCClient } from '../assets/LayaDCC/common/LayaDCCClient.js';
const { regClass, property, Loader } = Laya;
var keyevt = false;
let AllTest = class AllTest extends Laya.Script {
    constructor() {
        super(...arguments);
    ...
    onEnable() {
        this.out = this.owner.parent.getChildByName('TextArea');
        this.out.text = 'hello';
        ...
        if (!keyevt) {
            Laya.stage.on(Laya.Event.KEY_DOWN, (event) => {
...
2. 这个测试不需要服务器，apk中已经包含资源了
        ...
        let _txt = this.owner.getChildByName('Text');
    ...
    async xhr() {
        ...
        xhr.once(Laya.Event.COMPLETE, null, (p) => { debugger; console.log('eeeeeee' + p); });
    }
    async commonDown() {
        /**
         * android测试：
