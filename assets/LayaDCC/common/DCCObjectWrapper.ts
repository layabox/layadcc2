 ...
 * 混淆文件
 */
export class DCCObjectWrapper {
    version = 1;
    xorKey: Uint8Array | null = null;
    dataLength: number;
    static FLAG = new Uint8Array([108, 97, 121, 97, 100, 99, 99, 50]);//layadcc2

    static wrapObject(buff: ArrayBuffer, head: DCCObjectWrapper) {
        let headLen =
            8       //flag
            + 4      //version
            + 8      //key  固定为8
    ...
    static xorEncryptArrayBuffer(inputBuffer: ArrayBuffer, inputBufferOff: number, key: Uint8Array, outbuff: ArrayBuffer | null, outbuffOff: number) {
        if (key.length !== 8) {
