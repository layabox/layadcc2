...
export class GitFS {
    ...
    //读的防抖（静态，所有实例共享）
    private static _pending = new Map();
    ...
    async getTreeNode(objid: string, treeNode: TreeNode | null) {
        ...
        // 如果需要缓存，先设置 _pending
        let resolveFunc!: (value: TreeNode) => void;
        let rejectFunc!: (reason?: any) => void;
        if (key) {
            const pendingPromise = new Promise<TreeNode>((resolve, reject) => {
                resolveFunc = resolve;
                rejectFunc = reject;
            });
    ...
    async getBlobNode(objid: string | Uint8Array, encode: 'utf8' | 'buffer'): Promise<string | ArrayBuffer> {
        ...
        if (GitFS._pending.has(key)) {
            const cachedBuff = await GitFS._pending.get(key) as ArrayBuffer;
            if (encode == 'utf8') {
                return readUTF8(cachedBuff);
            }
            return cachedBuff;
        }

        // 先创建 Promise 并设置到 _pending，确保后续调用能命中缓存
        let resolveFunc!: (value: ArrayBuffer) => void;
        let rejectFunc!: (reason?: any) => void;
        const pendingPromise = new Promise<ArrayBuffer>((resolve, reject) => {
            resolveFunc = resolve;
            rejectFunc = reject;
        });
        GitFS._pending.set(key, pendingPromise);
    ...
    async visitAll(node: TreeNode, treecb: (cnode: TreeNode, entry: TreeEntry) => Promise<void>, blobcb: (entry: TreeEntry) => Promise<void>, inEntry: TreeEntry, openNode=true) {
        await treecb(node, inEntry);
        for await (const entry of node.entries) {
            if (entry.isDir) {
                try {
                    if (!entry.treeNode && openNode) await this.openNode(entry);
                    if(entry.treeNode) 
