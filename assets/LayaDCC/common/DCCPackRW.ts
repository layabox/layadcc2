...
export class DCCPackW {
    ...
    private indexObj:{[key:string]:ArrayBuffer}={};
    addObj(objid: string, buf: ArrayBuffer) {
        this.indexObj[objid]=buf.slice(0);
    }

    //从节点考虑的话，还是不要有状态了，不要用this
    pack(indexObj:{[key:string]:ArrayBuffer}|null){
        if(!indexObj)
            indexObj = this.indexObj;
        let indexes:IindexItem[]=[];
        let buffLen=0;        
        for(let obj in indexObj){
            let buff = indexObj[obj];
            if(buff) buffLen+=buff.byteLength;
        }
    ...
    mergeIndexAndContent(index:ArrayBuffer|IindexItem[], content:ArrayBuffer){
        let indexBuff:ArrayBuffer;
